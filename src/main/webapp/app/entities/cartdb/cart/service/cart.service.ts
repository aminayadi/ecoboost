...
type RestOf<T extends ICart | NewCart> = Omit<T, 'lastestModif'> & {
  lastestModif?: string | null;
};
...
export class CartService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/carts', 'cartdb');
  ...
  protected convertDateFromClient<T extends ICart | NewCart | PartialUpdateCart>(cart: T): RestOf<T> {
    return {
      ...cart,
      lastestModif: cart.lastestModif?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCart: RestCart): ICart {
    return {
      ...restCart,
      lastestModif: restCart.lastestModif ? dayjs(restCart.lastestModif) : undefined,
    };
  }
